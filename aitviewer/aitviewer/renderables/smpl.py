        ...
        **kwargs,
        ...
        if self.smpl_layer.model_type != "flame":
            if self.smpl_layer.model_type != "mano":
                # When hand poses are present, use the full skeleton (body+hands)
                # for local_to_global, otherwise just body.
                has_hands = self.poses_left_hand is not None and self.poses_right_hand is not None
                if has_hands:
                    all_poses = torch.cat([self.poses_root, self.poses_body, self.poses_left_hand, self.poses_right_hand], dim=-1)
                    ...
                    parents_for_l2g = self.skeleton[:, 0]
                global_oris = local_to_global(
                    all_poses,
                    parents_for_l2g,
                    output_format="rotmat",
                )
                # Trim to body-only joints to match self.joints from fk().
                if has_hands:
                    n_body = self.skeleton.shape[0]
                    global_oris = global_oris.reshape((self.n_frames, -1, 3, 3))[:, :n_body]
                    global_oris = global_oris.reshape((self.n_frames, n_body * 3 * 3))
            else:
                global_oris = local_to_global(
